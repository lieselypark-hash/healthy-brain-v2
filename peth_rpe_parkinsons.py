...
using the Parkinson dopamine variant agent.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot Parkinson-variant RPE traces aligned to cue onset and reward events for beginning/middle/end training phases.",
    )
    ...
    parser.add_argument("--out", type=str, default="results/rpe_peth_parkinsons_begin_mid_end.png")
    parser.add_argument("--counts_out", type=str, default="results/rpe_peth_parkinsons_event_counts.csv")
...
def compute_parkinsons_rpe(agent: A2CAgent, state: np.ndarray, reward: float,
    ...
    with torch.no_grad():
        ...
        v_t = v.squeeze()
        nv_t = torch.tensor(0.0, dtype=torch.float32) if done else nv.squeeze()
...
def plot_peth(phase_data: list[dict], out_path: str, pre: int, post: int) -> None:
    ...
    fig.suptitle("Parkinson variant: PETH-adjacent RPE profiles at beginning/middle/end of training", fontsize=13)
    fig.tight_layout()
